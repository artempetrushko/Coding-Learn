...
namespace Scripts
{
    public class PadHandbookView : MonoBehaviour
    {
        ...
        public void CreateThemeButtons(List<string> themes, Action<int> themeButtonPressedAction)
        {
            for (var i = 1; i <= themes.Count; i++)
            {
                var themeNumber = i;
                var themeButton = Instantiate(themeButtonPrefab, themeButtonsContainer.transform);
