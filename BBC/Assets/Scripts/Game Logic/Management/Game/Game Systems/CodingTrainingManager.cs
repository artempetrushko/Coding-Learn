...
namespace Scripts
{
    public class CodingTrainingManager : MonoBehaviour
    {
        ...
        {
            currentCodingTrainingInfos = ContentManager.GetCodingTrainingInfo(themeNumber, subThemeNumber);
        ...
        private void ShowTrainingContentPart(int trainingPartNumber)
        {
            ...
            if (selectedCodingTrainingPart.VideoTitles != "")
            {
                codingTrainingSectionView.CreateTrainingTextVideoPage(selectedCodingTrainingPart.Title, selectedCodingTrainingPart.Info, Resources.Load<VideoClip>(trainingVideoResourcesFolderPath + selectedCodingTrainingPart.VideoTitles), ShowingMode);
            }
            else
            {
