...
namespace UI.MainMenu
{
    public class LevelsMenuView : MonoBehaviour
    {
        ...
        public LevelsMenuView(LevelsMenuConfig levelsSectionConfig, LevelButton levelButtonPrefab)
        {
        ...
        {
            SetContentVisibilityAsync(_header, isVisible ? -1 : 1, SECTION_VISIBILITY_CHANGING_DURATION).Forget();
            SetContentVisibilityAsync(_levelButtonsContainer, isVisible ? 1 : -1, SECTION_VISIBILITY_CHANGING_DURATION).Forget();
            _levelThumbnailView.Thumbnail.DOFade(isVisible ? 1f : 0f, SECTION_VISIBILITY_CHANGING_DURATION).ToUniTask().Forget();
        ...
        {
            _levelTitle.text = levelConfig.Title.GetLocalizedString();
        ...
        private void CreateLevelButtons(int lastAvailableLevelNumber)
        {
            ...
            {
                var levelButton = Object.Instantiate(_levelButtonPrefab, _levelButtonsContainer.transform);
        ...
        private async UniTask ChangeLevelThumbnailAsync(AssetReference thumbnailReference)
        {
            ...
            _transitionThumbnailView.SetActive(true);
            _transitionThumbnailView.CanvasGroup.alpha = 1f;
            _transitionThumbnailView.Thumbnail.sprite = _levelThumbnailView.Thumbnail.sprite;
            _levelThumbnailView.Thumbnail.sprite = newThumbnail;

            await _transitionThumbnailView.CanvasGroup.DOFade(0f, _levelsSectionConfig.ThumbnailChangingDuration).AsyncWaitForCompletion();
            _transitionThumbnailView.SetActive(false);
        }
        ...
        {
            _levelDescriptionView.SetActive(true);

            var backgroundParts = _levelDescriptionView.BackgroundParts;
            ...
            _levelDescriptionView.SetDescriptionText(levelDescriptionText);
        }

        private async UniTask HideLevelDescriptionViewAsync()
        {
            _levelDescriptionView.SetDescriptionText("");
            ...
            _levelDescriptionView.SetActive(false);
        }
